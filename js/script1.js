window.addEventListener("load", function() {
    ...
    const aboutBlock = document.querySelectorAll(".about_block");
    const aboutBlockBtn = document.querySelector(".about_container_btn");
    let lastVisible = 2;

    aboutBlockBtn.addEventListener("click", function() {
        let showCount = 3;
        for (let i=lastVisible+1; i < aboutBlock.length && showCount > 0; i++, showCount--) {
            aboutBlock[i].classList.add("about_visible");
            lastVisible++;
        }
    });
